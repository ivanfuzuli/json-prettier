...
const {red, cyan, blue, green, magenta, bold} = colors;
...
class Dumper {
  ...
  generateDump(toDump, indent = '') {
    ...
    return startWith + `<div class="jw-expandable">
                          <div class="jw-btn-toggle"></div>
                          <div class="jw-dump">${dump}</div>
                          <span class="jw-dots jw-hidden">...</span>
                        </div>` + endWith;
  }
  ...
  makeArrowString(paramType, indent, key, valueDump) {
    ...
    return `<div class="jw-block">${keyPart} => ${valuePart}</div>`;
  }
}
